...
<div id="wrap">
	...
	<div id="container">
		...
		<div id="outbox">		
			...
			<div id="contents">
				<div id="mypage">
					<h2><strong>진행중 이벤트</strong><span>${edto.etitle}</span></h2>
					
					<div class="viewDivMt">
						<div class="viewHead">
							<div class="subject">
								<ul>
									<li>이벤트에 참가하여 행복을 만끽하세요.</li>
								</ul>
							</div>
							<div class="day">
								<p class="txt">이벤트 기간<span><fmt:formatDate value="${edto.stdate}" pattern="yyyy-MM-dd"/> ~ <fmt:formatDate value="${edto.enddate}" pattern="yyyy-MM-dd"/></span></p>
							</div>
						</div>
					...
					<script>
						...
						$(function(){
							$(".replyBtn").click(function(){
								...
								if($(".replyType").val().trim().length<1){
									alert("댓글을 입력하세요.");
									$(".replyType").val("");
									$(".replyType").focus();
									return;
								...
								$.ajax({
									...
									type:"post",
									data:{"eno":eno,"cpw":cpw,"ccontent":ccontent},
									dataType:"json",	// 리턴 받을 갑의 형태
									success:function(data){
										console.log("eno : "+data.eno);
										console.log(data.cpw);
										console.log("cno : "+data.cno);
										console.log(data.ccontent);
										console.log("cdate : "+data.cdate);
										...
										if(cpw == null){
											hdata += '<ul id="'+cno+'">';
											hdata += '<li class="name">'+id+' <span> ['+moment(cdate).format("YYYY-MM-DD HH:mm:ss")+']</span></li>';
											...
											hdata += '<li class="txt">';
											hdata += '<a href="password.html" class="passwordBtn"><span class="orange">※ 비밀글입니다.</span></a>';
											hdata += '</li>';
											hdata += '</ul>';
										}
							...
							// 댓글 삭제
							// 정적형태 ->화면에 표시가 되면 추가된 html소스에는 적용이 안됨.
							//$(".deleteBtn").click(funtion(){
							//})
							// 동적형태 -> 댓글을 추가하면 html소스가 추가되니까 추가적인 html소스에서도 적용이 가능 => 이렇게 해야함.
							$(document).on("click",".deleteBtn",function(){
								let cno = $(this).closest("ul").attr("id");
								console.log($(this).closest("ul").attr("id"));
								if(confirm(cno+"번 댓글을 삭제하시겠어요?")){
									$.ajax({
										url:"/event/cdelete",// 링크주소
										type:"post",
										data:{"cno":cno},
										dataType:"text",	// 리턴 받을 갑의 형태
										success:function(data){
											console.log("result : "+data);

											// 삭제 - html ul id로 html 코드 삭제
											$("#"+cno).remove();
							...
							$(document).on("click",".updateBtn",function(){
								if(chk == 1){
									alert("다른 댓글의 수정창이 열려있어요. 닫은 후 다시 시도하세요.");
									return;
								}
							...
							$(document).on("click",".saveBtn",function(){
								...
								$.ajax({
									...
									type:"post",
									data:{"eno":eno, "cno":cno, "ccontent":ccontent},
									dataType:"json",	// 리턴 받을 갑의 형태
									success:function(data){
										console.log("eno : "+data.eno);
										console.log(data.cpw);
										...
										let hdata = "";
											...
											hdata += '<li class="name">'+id+' <span> ['+moment(data.cdate).format("YYYY-MM-DD HH:mm:ss")+']</span></li>';
											hdata += '<li class="txt">'+ccontent+'</li>';
											hdata += '<li class="btn">';
											hdata += '<a  class="rebtn updateBtn">수정</a>&nbsp';
											hdata += '<a  class="rebtn deleteBtn">삭제</a>';
					...
					<div class="replyBox">
						...
						<ul>
							...
							<li class="btn">
								<a href="#" class="rebtn saveBtn">수정</a>
						...
						<ul id="${cdto.cno}">
							<li class="name">${cdto.mdto.id} <span>[${cdto.cdate}]</span></li>
							<li class="txt">${cdto.ccontent}</li>
							<c:if test="${session_id == cdto.mdto.id}">
							<li class="btn">
								<a  class="rebtn updateBtn">수정</a>
								<a  class="rebtn deleteBtn">삭제</a>
							</li>
						...
						<ul id="${cdto.cno}">
							<li class="name">${cdto.mdto.id} <span>[${cdto.cdate}]</span></li>
							<li class="txt">
								<a href="/event/pwCboard?cno=${cdto.cno}" class="passwordBtn"><span class="orange">※ 비밀글입니다.</span></a>
							</li>
						</ul>
						</c:if>
						</c:forEach>
